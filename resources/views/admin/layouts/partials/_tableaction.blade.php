@switch($type)
    @case(TYPE_STATUS_COLUMN)
        ...
        @case(TYPE_NEWITEMSTATUS_COLUMN)
            @if(method_exists($model,'uniqueKey'))             
                <label class="switch" for="newitemid_{{$model->{$model::uniqueKey()} }}">
                    <input type="checkbox" itemkey="{{$model->{$model::uniqueKey()} }}" action="{{ route($route) }}" class="SwitchNewitem" id="newitemid_{{$model->{$model::uniqueKey()} }}" @if( $model->newitem_status === ITEM_ACTIVE ) checked="true" @endif >
                    <span class="slider"></span>
                </label>            
            @else                 
                <label class="switch" for="newitemid_{{$model->{$model::primaryKey()} }}">
                    <input type="checkbox" itemkey="{{$model->{$model::primaryKey()} }}" action="{{ route($route) }}" class="SwitchNewitem" id="newitemid_{{$model->{$model::primaryKey()} }}" @if( $model->newitem_status === ITEM_ACTIVE ) checked="true" @endif >
                    <span class="slider"></span>
                </label>  
            @endif            
        @break
