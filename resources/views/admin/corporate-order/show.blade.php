            ...
            <table class="table table-bordered table-striped" id="orderTable">              
              <tbody>   
                ...
                <tr>
                    ...
                    <td>{{ ($model->payment_type === null) ? 'undefined' : $model->corporatePaymentTypes($model->payment_type) }}</td>    
                </tr>                   
                <tr>
                ...
                <tr>
                    <th>@lang('admincrud.Items')</th>
                    <td>
                    <table class="table table-condensed">
                    <thead>
                        <tr>
                            <th>@lang('admincrud.Voucher Number')</th>
                            <th>@lang('admincrud.Item Name')</th>
                            <th>@lang('admincrud.Ingredients')</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($model->items as $key => $value)
                        <tr>
                            <td>{{ ($value->voucher_number === null) ? '-' : $value->voucher_number }}</br></td>
                            <td>{{ ($value->item_name === null) ? '-' : $value->item_name }}</td>
                            <td>
                                @foreach($value->ingredients as $key => $ingredientName )
                                    <ul>
                                        <li>
                                            {{ ($ingredientName->ingredient_name === null) ? '' : $ingredientName->ingredient_name }}
                                        </li>
                                    </ul>
                                 @endforeach
                            <td>
