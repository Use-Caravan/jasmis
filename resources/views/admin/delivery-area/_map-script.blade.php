{{--  https://gist.github.com/knownasilya/89a32e572989f0aff1f8   Reference link --}}
...
function initMap() {
    ...
    if( $('#zone_type').val() == '') {
        ...
        if($('#zone_type').val() == {{DELIVERY_AREA_ZONE_CIRCLE}}) {            
            var zone_type = google.maps.drawing.OverlayType.CIRCLE
            ...
            @if($model->exists)
            console.log('{!! $model->zone_latlng !!}');
            $('#zone_latlng').val('{!! $model->zone_latlng !!}');
...
{
    /* console.log(circle); */
    $('#zone_type').val({{DELIVERY_AREA_ZONE_CIRCLE}}); 
...
{
    $('#zone_type').val({{DELIVERY_AREA_ZONE_POLYGON}});         
    var coordinates = [];              
