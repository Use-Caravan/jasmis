...
class CategoryLang extends CModel
{
     ...
     * @return $query
     */
    public static function selectTranslationByName($query,$name, $alias = '')
    {
        $self = new self();
        $category = new Category();
        $tableName = $self->table;
        ...
        $query->leftJoin("$tableName as $alias",function($query) use ($alias, $langForeignKey, $category, $name) {
            $query->on($category->getTable().".$langForeignKey", '=', "$alias.$langForeignKey")
            ->where("$alias.language_code","en")
            ->where("$alias.category_name", $name);
        })->addSelect($selects);
    }
}
